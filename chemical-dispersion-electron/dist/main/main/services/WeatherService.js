...
class WeatherService {
     ...
     */
    async getNWSCurrentWeather(latitude, longitude) {
        try {
            // Get grid point for coordinates
            const pointResponse = await axios_1.default.get(`${this.nwsBaseUrl}/points/${latitude},${longitude}`, {
                timeout: 10000,
            ...
            const obsResponse = await axios_1.default.get(`${this.nwsBaseUrl}/gridpoints/${forecastOffice}/${gridX},${gridY}/observations/latest`, {
                timeout: 10000,
                headers: { 'User-Agent': 'ChemicalDispersionApp/1.0' }
            });
     ...
     */
    async getOpenMeteoCurrentWeather(latitude, longitude) {
        try {
            const response = await axios_1.default.get(`${this.openMeteoBaseUrl}/forecast`, {
                params: {
                    latitude,
                    longitude,
        ...
        try {
            ...
            for (let i = 0; i < Math.min(24, hourly.time?.length || 0); i++) {
                const forecast = {
                    ...
                    stabilityClass: this.determineStabilityClass(hourly.windspeed_10m?.[i] || 5, hourly.temperature_2m?.[i] || 20),
     ...
     */
    determineStabilityClass(windSpeed, temperature) {
